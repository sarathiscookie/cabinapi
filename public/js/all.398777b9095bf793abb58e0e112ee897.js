...
$(function(){
    ...
    $('#tourname').change(function () {
        ...
        if(tourId != '') {
            ...
            $.ajax({
                type: "GET",
                url: '/mountainschool/tours/gettour/'+tourId+'?tours='+tours,
                success: function (data) {
                    ovelayLoading('remove');
                    $('#cabindtls').append(data);
                    $('#new_tour').html('<i class="fa fa-plus-circle fa-2x icon-primary" id="add_tour" data-tour="' + tourId + '" data-tours="' + tours + '"></i>');
                    $('#newBooking').show();
                }
            });
    ...
    $(document).on('click', '#add_tour', function(e) {
        ...
        tours = $('.tour-box').length + 1;
        console.log(tours);
        $('#add_tour').attr('data-tours', tours);

        getTour(tourId, tours);
        updateTours(tourId);
    });
    ...
    function updateTours(tourId) {
        $('.checkInCls').each(function(index) {
            ...
            $(this).attr('id', 'check_in' + tourId + (index+1));
            $(this).attr('name', 'check_in' + tourId + (index+1) + '[]');
        });

        $('.calendar').each(function(index) {
            $(this).attr('data-id', + tourId + (index+1));
        });

        $('.calendar-holiday').each(function(index) {
            $(this).attr('class', 'test test');
        });
    }
